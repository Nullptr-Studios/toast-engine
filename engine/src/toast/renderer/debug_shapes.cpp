...
void roundedShape(const glm::mat4& transform, float radius, float shaft, glm::vec4 color, bool fill, bool wire) {
	...
	auto point = [&](int ring, int segment) -> glm::vec3 {
		const float latitude = ring <= 8 ? -glm::half_pi<float>() + glm::half_pi<float>() * ring / 8
		    : glm::half_pi<float>() * (ring - 9) / 8;
		const float longitude = glm::two_pi<float>() * segment / segments;
		const float offset = ring <= 8 ? -shaft : shaft;
		return transform * glm::vec4(radius * std::cos(latitude) * std::cos(longitude),
		    radius * std::cos(latitude) * std::sin(longitude), radius * std::sin(latitude) + offset, 1.0f);
	};
	for (int i = 0; i < rings; ++i) {
		for (int j = 0; j < segments; ++j) {
			...
			if (wire) {
				if (j % 6 == 0) debugDrawLine(a, d, color);
				if (i == 8 || i == 9) debugDrawLine(a, b, color);
			}
		}
	}
}
...
void debugDrawCapsule(const glm::mat4& transform, float radius, float height, glm::vec4 color, bool fill) {
	ZoneScoped;
	if (!std::isfinite(radius) || !std::isfinite(height) || radius <= 0.0f || height < 2.0f * radius) return;
...
void debugDrawShapeBox(const glm::mat4& transform, glm::vec4 color, bool fill) {
	...
	for (int i = 0; i < 8; ++i) {
		corners[i] = transform * glm::vec4((i & 1) ? 0.5f : -0.5f, (i & 2) ? 0.5f : -0.5f,
		    (i & 4) ? 0.5f : -0.5f, 1.0f);
	}
	if (fill) {
		auto fill_color = color;
		fill_color.a *= 0.2f;
		constexpr int faces[6][4] = {{0,2,3,1},{4,5,7,6},{0,1,5,4},{2,6,7,3},{0,4,6,2},{1,3,7,5}};
	...
	for (int i = 0; i < 8; ++i) {
		for (int bit : {1, 2, 4}) {
			if (!(i & bit)) debugDrawLine(corners[i], corners[i | bit], color);
		}
	}
}
}
