...
public:
	...
	struct DrawPushConstants {
		glm::mat4 model;
		glm::vec4 color;
		glm::vec4 pbr_params;    // x = metallic, y = roughness, z = debug render mode (0 Lit, 1 ClusterHeatmap)
	};
...
private:
	void createResources(const renderer::VulkanCore& core, const ClusterLightingPass& cluster_lighting_pass);

	/// @brief Allocates+writes set 2 (clustered lighting buffers) once - buffer addresses are stable for
	/// ClusterLightingPass's lifetime, same reasoning as set 0
	void createClusterLightingResources(const renderer::VulkanCore& core, const ClusterLightingPass& cluster_lighting_pass);

	/// @brief Builds the 1x1 white albedo + flat-normal fallback textures/sampler/descriptor used for meshes
	/// that have no material, or whose material texture isn't uploaded yet
	...
	// Set 2: one descriptor set per frame-in-flight, bound to ClusterLightingPass's per-frame buffers
	std::vector<vk::raii::DescriptorSet> m_light_descriptor_sets;

	// per-material descriptor sets keyed by Material
	struct MaterialGpuBinding {
		vk::raii::DescriptorSet set = nullptr;
		vk::ImageView bound_view;
		vk::ImageView bound_normal_view;
	};
	...
	renderer::VulkanTexture m_default_texture;           // 1x1 white (albedo)
	renderer::VulkanTexture m_default_normal_texture;    // 1x1 flat normal (0.5, 0.5, 1.0)
