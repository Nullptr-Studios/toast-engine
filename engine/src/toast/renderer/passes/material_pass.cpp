...
void MaterialPass::createFrameSets() {
	...
	for (uint32_t i = 0; i < VulkanRenderer::k_frames_in_flight; ++i) {
		...
		// Set 0 is engine-reserved: every binding declared there is either the frame UBO or one of the
		// culled-lighting buffers ClusterLightingPass owns - never a material-authored value (see
		// MaterialRuntime::bakeValues(), which skips set-0 bindings entirely). Bind by declared name so a
		// material shader can opt into as many (or as few) of these as it actually uses
		std::vector<vk::DescriptorBufferInfo> buffer_infos;
		std::vector<vk::WriteDescriptorSet> writes;
		buffer_infos.reserve(m_root_runtime.reflection().bindings.size());
		...
		for (const auto& binding : m_root_runtime.reflection().bindings) {
			...
			if (binding.name == "gCamera") {
				...
				TOAST_WARN("Render", "MaterialPass '{}': unrecognized frame-set binding '{}', leaving unbound", m_name, binding.name);
				continue;
			}
			...
			buffer_infos.emplace_back(buffer, 0, size);
			writes.emplace_back(*m_frame_descriptor_sets[i], binding.binding, 0, 1, descriptor_type, nullptr, &buffer_infos.back());
		}
